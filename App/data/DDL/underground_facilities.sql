...
create table db_fac.data_underground_facilities_{provider_id} (
  id bigint not null
  ...
  , constraint data_underground_facilities_{provider_id}_PKC primary key (id)
) ;

-- インデックス付与
create index data_underground_facilities_{provider_id}_geom_idx
  on db_fac.data_underground_facilities_{provider_id}
  using GIST (geom);
