...
create table db_fac.data_utility_poles_{provider_id} (
  id bigint not null
  ...
  , qty integer
  , bury_depth integer not null
  , pole_hgt integer not null
  , pole_wd integer not null
  , occ_perm date
  , opt_attr text
  ...
  , constraint data_utility_poles_{provider_id}_PKC primary key (id)
) ;

-- インデックス付与
create index data_utility_poles_{provider_id}_geom_idx
  on db_fac.data_utility_poles_{provider_id}
  using GIST (geom);
